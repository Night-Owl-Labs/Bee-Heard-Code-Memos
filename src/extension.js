...
function activate(context) {
    ...
    const startRecordingCommand = vscode.commands.registerCommand('beeHeard.newCodeMemo', startRecording);
    const stopRecordingCommand = vscode.commands.registerCommand('beeHeard.endCodeMemo', stopRecording);
    const configureSavePathCommand = vscode.commands.registerCommand('beeHeard.configureSavePath', configureSavePath);
    const resetSavePathCommand = vscode.commands.registerCommand('beeHeard.resetSavePath', resetSavePath);
    const setMaxDurationCommand = vscode.commands.registerCommand('beeHeard.setMaxDuration', setMaxDuration);
...
function configureSavePath() {
    vscode.window.showOpenDialog({ canSelectFolders: true, canSelectMany: false }).then(folderUri => {
        if (folderUri && folderUri[0]) {
            const newPath = folderUri[0].fsPath + '/code_memos';
            vscode.workspace.getConfiguration().update('beeHeard.savePath', newPath, vscode.ConfigurationTarget.Global);
            vscode.window.showInformationMessage(`Code Memos save path set to: ${newPath}`);
        }
    });
}
...
function resetSavePath() {
    ...
    vscode.workspace.getConfiguration().update('beeHeard.savePath', defaultPath, vscode.ConfigurationTarget.Global);
    vscode.window.showInformationMessage('Code Memos save path reset to default workspace folder');
}

function getSavePath() {
    const configuredPath = vscode.workspace.getConfiguration().get('beeHeard.savePath');
    return configuredPath.replace('${workspaceFolder}', vscode.workspace.workspaceFolders[0].uri.fsPath);
}

function startRecording() {
    if (recording) {
        vscode.window.showErrorMessage('Code Memo - Recording is already in progress');
        return;
    }
    ...
    recordingMessage = vscode.window.setStatusBarMessage('$(record) Recording...');
    ...
    const maxDuration = vscode.workspace.getConfiguration().get('beeHeard.maxDuration');
    if (maxDuration > 0) {
...
function setMaxDuration() {
    vscode.window.showInputBox({ prompt: 'The maximum recording duration in seconds (default is 300 seconds, 5 minutes). Set to 0 for unlimited recording.' }).then(value => {
        const maxDuration = parseInt(value, 10);
        if (!isNaN(maxDuration)) {
            vscode.workspace.getConfiguration().update('beeHeard.maxDuration', maxDuration, vscode.ConfigurationTarget.Global);
            vscode.window.showInformationMessage(`Code Memo - Maximum recording duration set to: ${maxDuration} seconds`);
        } else {
            vscode.window.showErrorMessage('Invalid input for maximum recording duration');
        }
    });
}
